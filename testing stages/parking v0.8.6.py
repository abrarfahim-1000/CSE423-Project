...
# Collision cooldown
collision_cooldown_frames = 60  # Cooldown period in frames (e.g., 30 frames at 60 FPS ~ 0.5 seconds)
...
def draw_health_bar():
    if current_level != 0:  # Only draw health bar if not in level 0
...
def draw_fuel_bar():
    if current_level != 0:  # Only draw fuel bar if not in level 0
        global fuel
...
def advance_to_next_level():
    """
    Move to the next level if the player completes the current level.
    """
    global current_level, current_level_parking_spots, current_level_parked_cars, current_level_obstacles, player_pos, player_angle, current_level_road, game_state, frames_since_last_collision
...
def update_game_state():
    """
    Update the game state, checking for parking success and advancing levels.
    """
    global game_state

    if game_state == "PLAY" and check_parking_success():
...
def update_car_position():
    ...
    if forward_pressed or backward_pressed:
        ...
        if forward_pressed:
            ...
            for car in current_level_parked_cars:
                if check_vehicle_collision(new_pos_forward, player_angle, car_model, car["pos"], car["angle"], car["model"], check_sphere=front_sphere):
        ...
        if backward_pressed:
            ...
            for car in current_level_parked_cars:
                if check_vehicle_collision(new_pos_backward, player_angle, car_model, car["pos"], car["angle"], car["model"], check_sphere=back_sphere):
                    back_collision = True
                    break
    ...
    if new_pos != player_pos:  # Move if direction is clear
        ...
        # Deduct health only if pressing into a colliding direction and not in level 0
    ...
    # Fuel logic
    if current_level != 0:  # Only apply fuel logic if not in level 0
        if not game_over:
