...
level1_obstacles = [
    # Road 1: Vertical road (-850, -700, -850, 990)
    # Left side cones - placed 150 units from road with 250 unit spacing
    {"type": "cone", "pos": [-1000, 0, -600]},  # 150 units to the left of road
    {"type": "cone", "pos": [-1000, 0, -350]},  # 250 unit spacing
    {"type": "cone", "pos": [-1000, 0, -100]},  # 250 unit spacing
    {"type": "cone", "pos": [-1000, 0, 150]},  # 250 unit spacing
    {"type": "cone", "pos": [-1000, 0, 400]},  # 250 unit spacing
    {"type": "cone", "pos": [-1000, 0, 650]},  # 250 unit spacing
    {"type": "cone", "pos": [-1000, 0, 900]},  # 250 unit spacing

    # Right side barriers - placed 150 units from road with 350 unit spacing
    {"type": "barrier", "pos": [-700, 0, -550], "angle": 90},  # 150 units to the right of road
    {"type": "barrier", "pos": [-700, 0, -200], "angle": 90},  # 350 unit spacing
    {"type": "barrier", "pos": [-700, 0, 150], "angle": 90},  # 350 unit spacing
    {"type": "barrier", "pos": [-700, 0, 500], "angle": 90},  # 350 unit spacing
    # {"type": "barrier", "pos": [-700, 0, 850], "angle": 90},   # 350 unit spacing

    # Road 2: Horizontal road (-750, 850, 1000, 850)
    # Upper side cones - placed 150 units from road with 350 unit spacing
    {"type": "cone", "pos": [-700, 0, 690]},  # 150 units above road
    {"type": "cone", "pos": [-350, 0, 690]},  # 350 unit spacing
    {"type": "cone", "pos": [0, 0, 690]},  # 350 unit spacing
    {"type": "cone", "pos": [350, 0, 690]},  # 350 unit spacing
    {"type": "cone", "pos": [700, 0, 690]},  # 350 unit spacing

    # Lower side barriers - placed 150 units from road with 450 unit spacing
    {"type": "barrier", "pos": [-600, 0, 1000], "angle": 0},  # 150 units below road
    {"type": "barrier", "pos": [-150, 0, 1000], "angle": 0},  # 450 unit spacing
    {"type": "barrier", "pos": [300, 0, 1000], "angle": 0},  # 450 unit spacing
    {"type": "barrier", "pos": [750, 0, 1000], "angle": 0}  # 450 unit spacing
]
...
# Extended obstacles list with cones and barriers placed along the roads with bigger intervals and farther from roads
level2_obstacles = [
    # # Original obstacles
    ...
    # Road 1: Vertical road (300, 1000) to (300, 300)
    # Left side cones - increased spacing and moved farther from road
    {"type": "cone", "pos": [150, 0, 950]},  # Moved to 80 units from road instead of 50
    {"type": "cone", "pos": [150, 0, 700]},  # Increased interval to 250 units
    {"type": "cone", "pos": [150, 0, 450]},  # Increased interval to 250 units

    # Right side barriers - increased spacing and moved farther from road
    {"type": "barrier", "pos": [450, 0, 900], "angle": 90},  # Moved to 110 units from road instead of 80
    {"type": "barrier", "pos": [450, 0, 550], "angle": 90},  # Increased interval to 350 units

    # Road 2: Horizontal road (440, 160) to (-850, 160)
    # Upper side cones - increased spacing and moved farther from road
    {"type": "cone", "pos": [350, 0, 0]},  # Moved to 80 units from road instead of 50
    {"type": "cone", "pos": [50, 0, 0]},  # Increased interval to 350 units
    {"type": "cone", "pos": [-350, 0, 0]},  # Increased interval to 350 units
    # {"type": "cone", "pos": [-700, 0, 80]},   # Increased interval to 350 units

    # Lower side barriers - increased spacing and moved farther from road
    # {"type": "barrier", "pos": [250, 0, 240], "angle": 90},  # Moved to 80 units from road instead of 50
    {"type": "barrier", "pos": [-250, 0, 300], "angle": 0},  # Increased interval to 500 units
    {"type": "barrier", "pos": [-750, 0, 300], "angle": 0},  # Increased interval to 500 units

    # Road 3: Vertical road (-710, 20) to (-710, -500)
    # Left side cones - increased spacing and moved farther from road
    {"type": "cone", "pos": [-860, 0, 0]},  # Moved to 80 units from road instead of 50
    {"type": "cone", "pos": [-860, 0, -250]},  # Increased interval to 250 units
    {"type": "cone", "pos": [-860, 0, -500]},  # Increased interval to 250 units

    # Right side barriers - increased spacing and moved farther from road
    {"type": "barrier", "pos": [-560, 0, -50], "angle": 90},  # Moved to 80 units from road instead of 50
    {"type": "barrier", "pos": [-560, 0, -300], "angle": 90},  # Increased interval to 250 units

    # Road 4: Horizontal road (-850, -500) to (550, -500)
    # Upper side cones - increased spacing and moved farther from road
    {"type": "cone", "pos": [-800, 0, -700]},  # Moved to 80 units from road instead of 50
    {"type": "cone", "pos": [-450, 0, -700]},  # Increased interval to 350 units
    {"type": "cone", "pos": [-100, 0, -700]},  # Increased interval to 350 units
    {"type": "cone", "pos": [250, 0, -700]},  # Increased interval to 350 units

    # Lower side barriers - increased spacing and moved farther from road
    # {"type": "barrier", "pos": [-600, 0, -420], "angle": 90},  # Moved to 80 units from road instead of 50
    {"type": "barrier", "pos": [-150, 0, -350], "angle": 0},  # Increased interval to 450 units
    {"type": "barrier", "pos": [300, 0, -350], "angle": 0},  # Increased interval to 450 units
]
...
player_angle3 = 180
# Creating a sprawling road network that uses the full game world space
# Format: (x1, z1, x2, z2) for each road segment
road3 = [
    (800, 980, 800, 500),  # First vertical segment (starting point)
    (940, 500, -340, 500),  # First horizontal segment (left turn) - expanded length
    (-200, 500, -200, -200),  # Second vertical segment (downward) - expanded length
    (-340, -200, 640, -200),  # Second horizontal segment (right turn) - expanded length
    (500, -340, 500, -640),  # Third vertical segment (downward) - expanded length
    (500, -500, -950, -500),  # Third horizontal segment (left turn) - expanded length
    # (-850, -700, -850, -950)   # Fourth vertical segment (final stretch)
]

# Spacing out parked cars throughout the world
level3_parked_cars = [
...
# Extended obstacles list with cones and barriers placed along the roads with wider spacing
level3_obstacles = [
    # Road 1: Vertical road (800, 950) to (800, 500)
    # Left side cones - 200 units from road with 300 unit spacing
    {"type": "cone", "pos": [650, 0, 930]},  # 200 units from road
    {"type": "cone", "pos": [650, 0, 630]},  # 300 unit spacing

    # Right side barriers - 200 units from road with 400 unit spacing
    {"type": "barrier", "pos": [950, 0, 880], "angle": 90},  # 200 units from road
    {"type": "barrier", "pos": [950, 0, 580], "angle": 90},  # 300 unit spacing

    # Road 2: Horizontal road (800, 500) to (-200, 500)
    # Upper side cones - 200 units from road with 400 unit spacing
    {"type": "cone", "pos": [600, 0, 330]},  # 200 units from road
    {"type": "cone", "pos": [200, 0, 330]},  # 400 unit spacing
    # {"type": "cone", "pos": [-150, 0, 330]},  # 350 unit spacing

    # Lower side barriers - 200 units from road with 400 unit spacing
    {"type": "barrier", "pos": [500, 0, 650], "angle": 0},  # 200 units from road
    {"type": "barrier", "pos": [100, 0, 650], "angle": 0},  # 400 unit spacing
    {"type": "barrier", "pos": [-150, 0, 650], "angle": 0},  # 250 unit spacing

    # Road 3: Vertical road (-200, 500) to (-200, -200)
    # Left side cones - 200 units from road with 300 unit spacing
    {"type": "cone", "pos": [-350, 0, 400]},  # 200 units from road
    {"type": "cone", "pos": [-350, 0, 100]},  # 300 unit spacing
    {"type": "cone", "pos": [-350, 0, -150]},  # 250 unit spacing

    # Right side barriers - 200 units from road with 300 unit spacing
    # {"type": "barrier", "pos": [-50, 0, 400], "angle": 90},    # 200 units from road
    {"type": "barrier", "pos": [-50, 0, 200], "angle": 90},  # 300 unit spacing
    # {"type": "barrier", "pos": [-50, 0, -200], "angle": 90},   # 200 unit spacing

    # Road 4: Horizontal road (-200, -200) to (500, -200)
    # Upper side cones - 200 units from road with 350 unit spacing
    {"type": "cone", "pos": [-600, 0, -370]},  # 200 units from road
    {"type": "cone", "pos": [-300, 0, -370]},  # 350 unit spacing
    {"type": "cone", "pos": [100, 0, -370]},  # 200 unit spacing

    # Lower side barriers - 200 units from road with 350 unit spacing
    {"type": "barrier", "pos": [100, 0, -50], "angle": 0},  # 200 units from road
    {"type": "barrier", "pos": [400, 0, -50], "angle": 0},  # 300 unit spacing

    # Road 5: Vertical road (500, -200) to (500, -700)
    # Left side cones - 200 units from road with 300 unit spacing
    # {"type": "cone", "pos": [300, 0, -350]},    # 200 units from road
    # {"type": "cone", "pos": [300, 0, -650]},    # 300 unit spacing

    # Right side barriers - 200 units from road with 300 unit spacing
    {"type": "barrier", "pos": [650, 0, -100], "angle": 90},  # 200 units from road
    {"type": "barrier", "pos": [650, 0, -550], "angle": 90},  # 300 unit spacing

    # Road 6: Horizontal road (500, -700) to (-850, -700)
    # Upper side cones - 200 units from road with 400 unit spacing
    # {"type": "cone", "pos": [300, 0, -900]},    # 200 units from road
    # {"type": "cone", "pos": [-100, 0, -900]},   # 400 unit spacing
    # {"type": "cone", "pos": [-500, 0, -900]},   # 400 unit spacing
    # {"type": "cone", "pos": [-800, 0, -900]},   # 300 unit spacing

    # Lower side barriers - 200 units from road with 450 unit spacing
    {"type": "barrier", "pos": [300, 0, -650], "angle": 0},  # 200 units from road
    {"type": "barrier", "pos": [-150, 0, -650], "angle": 0},  # 450 unit spacing
    {"type": "barrier", "pos": [-600, 0, -650], "angle": 0}  # 450 unit spacing

    # Road 7: Vertical road (-850, -700) to (-850, -950)
    # Left side cones - 200 units from road with 300 unit spacing
    # {"type": "cone", "pos": [-1050, 0, -800]},   # 200 units from road

    # # Right side barriers - 200 units from road with 300 unit spacing
    # {"type": "barrier", "pos": [-650, 0, -850], "angle": 90}, # 200 units from road
]

# Adding multiple parking spots with more space between them
level3_parking_spots = [
    # {"pos": [-850, 0, -1000], "angle": 180, "occupied": False},
    {"pos": [-850, 0, -850], "angle": 180, "occupied": False}
    # {"pos": [-550, 0, -1000], "angle": 180, "occupied": False}
]
...
sphere_radii = {
    ...
    'truck': {'front': 50, 'back': 50, 'distance': 60},
...
def check_parking_success():
    ...
    for spot in current_level_parking_spots:
        ...
        if fully_inside:
            ...
            # angle_tolerance = 15  # Allow a small tolerance for misalignment
