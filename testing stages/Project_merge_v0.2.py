...
rotation_speed = 5
frame_counter = 0
first_person_view= False  # Flag for first-person view
fovY = 120
GRID_LENGTH = 1000
...
def mouseListener(button, state, x, y):
    global third_person_view, game_state, first_person_view
    ...
    if game_state == "PLAY":          
     if button == GLUT_RIGHT_BUTTON and state == GLUT_DOWN:
            first_person_view= not first_person_view  # Toggle first-person view
            print("Right mouse button clicked. Switching to first-person view...")
def setupCamera():
    global camera_pos,camera_angle,player_pos,first_person_view, car_angle
    ...
    if first_person_view:
        ...
        head_x = px+ math.cos(rad)*70  # player er body r x theke 60 shamne chokh, naile body r bhitor part dekha jabe. 
        head_y = py + math.sin(rad) *70
        head_z = 90
        look_x = px + math.cos(rad) *150
        look_y = py + math.sin(rad) *150
        gluLookAt(
